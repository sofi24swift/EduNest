...
export default function App() {
  ...
  const handleSelectScreen = (screen: ScreenType, argId?: string) => {
    ...
    if (!registeredUser && screen !== 'registration') {
      setActiveScreen('registration');
      setNavWarning("გთხოვთ, შექმნათ საფირმო პროფილი iPhone სიმულატორში, სანამ ნავიგაციას გააგრძელებთ!");
  ...
  return (
    <div id="edu-nest-app" className="min-h-screen bg-[#0f172a] text-slate-100 flex items-center justify-center p-4 select-none relative overflow-hidden">
      ...
      <div className="relative flex flex-col items-center w-full max-w-sm py-4">
        
        {/* Onboarding Visual Warning Banner */}
        {navWarning && (
          <div role="alert" className="mb-4 w-full p-3.5 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-200 text-xs font-semibold leading-relaxed shadow-lg animate-pulse text-center">
